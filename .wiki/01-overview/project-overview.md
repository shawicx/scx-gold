...
| 路由 | 页面 | 文件 | 能力 |
| ---- | ---- | ---- | ---- |
| `/` | 涨停候选筛选器 | `src/pages/ScreenerPage.tsx` | 实时筛选涨停候选股，线索标签，高亮卡片 + 明细表格，大盘指数条 + 板块涨跌排行，个股详情抽屉 |
...
1. **单一数据源（后端 service 层）**：
   - 所有业务数据（股票/指数/板块/搜索/关注/分析/黄金/配置/认证/运维）→ 后端 `scx-stock-api`
   - 涨停筛选器股票列表经 `/api/v1/stock/list` 分页拉取（含主力资金/行业字段），由 `stockListAdapter.ts` 适配
